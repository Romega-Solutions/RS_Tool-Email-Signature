...
export const GET: APIRoute = () => {
  const sendWebhookConfigured = Boolean(process.env.EASYCOMMS_WEBHOOK_URL || process.env.EMAIL_SIGNATURE_WEBHOOK_URL);
  const automationSchema = {
    ...
    sendWebhook: {
      ready: sendWebhookConfigured,
      envVars: ["EASYCOMMS_WEBHOOK_URL", "EMAIL_SIGNATURE_WEBHOOK_URL"],
      optionalTokenEnvVars: ["EASYCOMMS_WEBHOOK_TOKEN", "EMAIL_SIGNATURE_WEBHOOK_TOKEN"],
    },
    n8n: {
      callbackAuditEnvVars: ["N8N_URL", "N8N_API_KEY", "N8N_AUDIT_TABLE_ID"],
    },
    exampleEnvelope,
  };
